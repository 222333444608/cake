<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="蛋糕种类" prop="dangaozhonglei">
          <el-input v-model="ruleForm.dangaozhonglei" 
              placeholder="蛋糕种类" clearable  :readonly="ro.dangaozhonglei"></el-input>
        </el-form-item>
        <div v-else>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dangaozhonglei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dangaozhongleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
