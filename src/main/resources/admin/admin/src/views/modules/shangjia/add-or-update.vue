<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.dianpufengmian" label="店铺封面" prop="dianpufengmian">
          <file-upload
          tip="点击上传店铺封面"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.dianpufengmian?ruleForm.dianpufengmian:''"
          @change="dianpufengmianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.dianpufengmian" label="店铺封面" prop="dianpufengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.dianpufengmian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shenfenzheng = true;
          ...
          if(o=='dianpufengmian'){
            this.ruleForm.dianpufengmian = obj[o];
	    this.ro.dianpufengmian = true;
            continue;
          }
          if(o=='money'){
	...
	if(this.ruleForm.dianpufengmian!=null) {
		this.ruleForm.dianpufengmian = this.ruleForm.dianpufengmian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    dianpufengmianUploadChange(fileUrls) {
	this.ruleForm.dianpufengmian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
