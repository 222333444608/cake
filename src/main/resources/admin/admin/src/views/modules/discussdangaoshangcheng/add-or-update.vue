...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `discussdangaoshangcheng/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discussdangaoshangcheng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discussdangaoshangchengCrossAddOrUpdateFlag = false;
						 this.parent.search();
